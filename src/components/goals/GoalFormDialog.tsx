...
const GoalFormDialog: React.FC<GoalFormDialogProps> = ({ open, onOpenChange }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((!formData.linkedVaults || formData.linkedVaults.length === 0) && 
      ...
      toast.error('Selecione pelo menos um cofre ou investimento para vincular à meta.');
      return;
    }
              ...
              {vaults.length === 0 ? (
                ...
                vaults.map((vault) => (
                  <div key={vault.id} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`vault-${vault.id}`} className="text-sm font-normal cursor-pointer">
              ...
              {investments.length === 0 ? (
                ...
                investments.map((investment) => (
                  <div key={investment.id} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`investment-${investment.id}`} className="text-sm font-normal cursor-pointer">
